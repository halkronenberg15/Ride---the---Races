...
const adaptive=readFileSync(new URL('../src/engine/adaptiveTraining40251.ts',import.meta.url),'utf8');for(const token of ['SELECTION_WEIGHTS','purposeMatch:40','Unknown — insufficient evidence','Recovery recommended','fourWeekPreview','replaceWithAlternative','No curated workout'])assert.match(adaptive,new RegExp(token));assert.doesNotMatch(adaptive,/machine learning|artificial intelligence|predicted FTP gain/i)
