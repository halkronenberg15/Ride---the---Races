...
type Props={model:RoadModel;position:CanonicalCoursePosition;currentResistance:string;nextResistance:string;formatDistance:(km:number)=>string;formatTime:(seconds:number)=>string}
...
export function ClimbProfile4023({model,position,currentResistance,nextResistance,formatDistance,formatTime}:Props){
 ...
 const riderKm=climb.startDistance+position.climbCoordinate*climbLength,rider=xy(Math.max(windowStart,Math.min(windowEnd,riderKm)))
 ...
 const summit=(position.climbCompletion??0)===100
 return h('section',{className:'climb-profile-4023','aria-label':`${climb.name} Climb View`},
  h('header',null,h('strong',null,climb.name),h('span',null,summit?'SUMMIT':`${Math.round(position.climbCompletion??0)}%`),h('span',null,`${formatDistance(position.distanceToSummit??0)} · ${formatTime(position.timeToSummit??0)}`)),
  h('div',{className:'climb-guidance'},
   h('span',null,h('small',null,'CURRENT'),h('strong',null,`${position.currentGradientSection?.gradient.toFixed(1)??'—'}%`),h('b',null,`${currentResistance} resistance`)),
   ...
   h('span',{className:'change'},h('small',null,(position.distanceToNextGradientBoundary??0)<.005?'CHANGE':'CHANGE IN'),h('strong',null,(position.distanceToNextGradientBoundary??0)<.005?'NOW':formatDistance(position.distanceToNextGradientBoundary??0)))),
  h('div',{className:'climb-svg-region'},
   ...
   h('span',{className:'climb-rider','data-climb-coordinate':position.climbCoordinate.toFixed(6),style:{left:`${clamp(rider.x/100)*100}%`,top:`${rider.y}%`}},'🚴')))
