...
export function equipmentForDevices(devices:CareerState['rider']['devices']):CareerState['equipment']{
 if(devices.includes('Peloton'))return {activeEquipmentId:'peloton-baseline-bike',instances:[{id:'peloton-baseline-bike',name:'Peloton manual bike',manufacturer:'Peloton',modelFamily:'Bike',resistanceControl:'manual',powerAvailable:true,cadenceAvailable:true,resistanceAvailable:true,calibrationProfileId:'peloton-bike-manual-reference',calibrationConfidence:'BASELINE'}]}
 if(devices.includes('Wahoo')||devices.includes('Zwift'))return {activeEquipmentId:'smart-equipment',instances:[{id:'smart-equipment',name:'Smart equipment',manufacturer:'Other',modelFamily:'Controllable foundation',resistanceControl:'controllable',powerAvailable:true,cadenceAvailable:true,resistanceAvailable:false,calibrationConfidence:'UNAVAILABLE'}]}
 if(devices.includes('Manual only'))return {activeEquipmentId:'generic-manual-bike',instances:[{id:'generic-manual-bike',name:'Manual bike',manufacturer:'Other',modelFamily:'Uncalibrated',resistanceControl:'manual',powerAvailable:false,cadenceAvailable:true,resistanceAvailable:true,calibrationConfidence:'UNAVAILABLE'}]}
 return {activeEquipmentId:null,instances:[]}
}
export function migrateCareer(saved:Partial<CareerState>):CareerState{const base=createInitialCareer(),legacyTour={currentStage:saved.season?.currentStage??base.races.tour.currentStage,completedStages:saved.season?.completedStages??base.races.tour.completedStages},fallbackEquipment=equipmentForDevices(saved.rider?.devices??[]);return {...base,...saved,schemaVersion:3,onboardingComplete:saved.onboardingComplete??true,rider:{...base.rider,...saved.rider},equipment:{...fallbackEquipment,...saved.equipment,instances:saved.equipment?.instances??fallbackEquipment.instances},season:{...base.season,...saved.season},races:{tour:{...legacyTour,...saved.races?.tour},vuelta:{...base.races.vuelta,...saved.races?.vuelta}},trainingHistory:saved.trainingHistory??[],health:{...base.health,...saved.health},rideHistory:saved.rideHistory??[],settings:{...base.settings,...saved.settings}}}
