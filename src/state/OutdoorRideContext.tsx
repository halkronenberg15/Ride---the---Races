...
type Value={ride:ActiveOutdoorRide|null;elapsed:number;start:(assignmentId:string)=>void;pause:()=>void;resume:()=>void;requestCompletion:(endedEarly:boolean)=>void;clear:()=>void}
...
export function OutdoorRideProvider({children}:{children:React.ReactNode}){
 ...
 const value=useMemo<Value>(()=>({ride,elapsed:ride?outdoorElapsed(ride,now):0,start:assignmentId=>setRide(current=>current??startOutdoorRide(assignmentId,Date.now())),pause:()=>setRide(current=>current?pauseOutdoorRide(current,Date.now()):null),resume:()=>setRide(current=>current?resumeOutdoorRide(current,Date.now()):null),requestCompletion:endedEarly=>setRide(current=>current?{...pauseOutdoorRide(current,Date.now()),completionRequested:endedEarly?'EARLY':'FULL'}:null),clear:()=>setRide(null)}),[now,ride])
