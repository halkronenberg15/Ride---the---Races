...
export default function SeasonCalendarScreen({ season, currentRace, onBack, onOpenRace }: { season: Season; currentRace?: string; onBack: () => void; onOpenRace: (raceId: string) => void }) {
  ...
  const {career,endSeason}=useCareer(),{account}=useAuth()
  ...
  return <section className="season-calendar-screen">
    ...
    <section className="dashboard-card"><h2>Season lifecycle</h2>{career.season.closure.status==='ENDED'?<p>Season ended {new Date(career.season.closure.endedAt!).toLocaleDateString()}. Jean’s Season Review and Off-Season Goals is next; plan generation arrives in Alpha 4.0.25.</p>:<><p>Ending freezes results, preserves stages and rides, archives the season, and starts the locked Off-Season handoff. It never rewrites classifications.</p><button type="button" disabled={career.season.completedStages.length<21&&account?.role!=='owner'} onClick={()=>window.confirm('END SEASON? Competitive state will be frozen and archived. Rides remain available; classifications will not be rewritten.')&&endSeason(career.season.completedStages.length<21)}>END SEASON{career.season.completedStages.length<21?' (OWNER EARLY OVERRIDE)':''}</button></>}</section>
