...
import { canUsePreviewDataTransfer } from '../services/previewDeployment.ts'
...
export default function SettingsScreen({ onBack }: { onBack: () => void }) {
  ...
  const {account,isAuthenticated}=useAuth(),{ride}=useActiveRide(),previewTransferEnabled=canUsePreviewDataTransfer(isAuthenticated,account?.role)
  const { settings } = career
  const [preview,setPreview]=useState<ImportPreview|null>(null),[error,setError]=useState(''),[selectedFilename,setSelectedFilename]=useState(''),[confirmed,setConfirmed]=useState(false),[backup]=useState(()=>account&&previewTransferEnabled?readRecoveryBackup(account,true):null)
  ...
  const exportCareer=()=>{if(!account||!previewTransferEnabled)return;try{setError('');const payload=buildCareerExport(account,career,ride,packageMetadata.version,new Date().toISOString(),true);download(safeExportFilename(career.rider.name,payload.exportedAt.slice(0,10)),serializeCareerExport(payload))}catch(value){setError(value instanceof Error?value.message:'Export failed.')}}
  ...
  const importCareer=()=>{if(!account||!preview||!previewTransferEnabled)return;try{commitCareerImport(account,career,ride,preview,confirmed,new Date().toISOString(),true);location.reload()}catch(value){setError(value instanceof Error?value.message:'Import failed.')}}
  const restore=()=>{if(!account||!previewTransferEnabled||!window.confirm('Restore the pre-import career backup and replace the current imported career?'))return;try{restoreRecoveryBackup(account,true,true);location.reload()}catch(value){setError(value instanceof Error?value.message:'Restore failed.')}}
  ...
  return (
    <section className="data-screen settings-screen">
      ...
      {previewTransferEnabled&&account&&<section className="settings-card preview-transfer" aria-labelledby="preview-transfer-title"><div><p className="eyebrow">LOCAL CAREER BACKUP</p><h2 id="preview-transfer-title">CAREER DATA TRANSFER</h2><p>Each browser and home-screen installation may have separate local career storage. Export a JSON backup from the installation with your career, then import it here after signing in locally.</p></div><p><strong>This is a manual transfer, not cloud synchronization.</strong> Files contain personal training information, are processed locally, are never uploaded by RtR, contain no credentials, and should be stored securely. Files are not encrypted.</p>{activeRunning&&<p className="transfer-error" role="alert">Pause or end the active ride before exporting or importing career data.</p>}<div className="transfer-actions"><button type="button" disabled={activeRunning} onClick={exportCareer}>Export My RtR Career</button><label className="file-picker">Import RtR Career<input type="file" accept="application/json,.json" disabled={activeRunning} onChange={selectFile}/></label></div>{selectedFilename&&<p className="selected-file"><strong>Selected:</strong> {selectedFilename}</p>}{error&&<p className="transfer-error" role="alert">{error}</p>}{preview&&<article className="import-preview"><h3>Import preview</h3><dl>{Object.entries({'Rider':`${preview.summary.riderName} · Rider #${preview.summary.riderNumber}`,'Source app':preview.summary.sourceApplicationVersion,'Source schema':preview.summary.sourceSchemaVersion,'Export date':new Date(preview.summary.exportedAt).toLocaleString(),'Current season':preview.summary.currentSeason,'Completed stages':preview.summary.completedStages,'Ride history':preview.summary.rideHistory,'Archived seasons':preview.summary.archivedSeasons,'FTP':preview.summary.ftp===null?'Unavailable':`${preview.summary.ftp} W`,'Series / programs':`${preview.summary.series} · ${preview.summary.programs}`,'Active ride':preview.summary.activeRideIncluded?'Compatible paused ride included':'Not included','Destination':'Current signed-in career will be replaced'}).map(([key,value])=><div key={key}><dt>{key}</dt><dd>{value}</dd></div>)}</dl>{preview.summary.identityDiffers&&<p className="identity-warning"><strong>Different rider:</strong> this replaces {career.rider.name} · Rider #{career.rider.number} with {preview.summary.riderName} · Rider #{preview.summary.riderNumber}. Histories will not be merged.</p>}<p>This will replace the career data associated with the currently signed-in local account. Authentication credentials will not be imported.</p><label className="confirm-transfer"><input type="checkbox" checked={confirmed} onChange={event=>setConfirmed(event.target.checked)}/> I understand this replaces this account’s career and preserves its sign-in credentials.</label><button type="button" disabled={!confirmed||activeRunning} onClick={importCareer}>Confirm and replace career</button></article>}{backup&&<article className="backup-summary"><h3>Restore Pre-Import Backup</h3><p>{new Date(backup.createdAt).toLocaleString()} · {backup.rider.name} · Rider #{backup.rider.number} · Schema {backup.schemaVersion}</p><button type="button" disabled={activeRunning} onClick={restore}>Restore Pre-Import Backup</button></article>}</section>}
    </section>
  )
}
