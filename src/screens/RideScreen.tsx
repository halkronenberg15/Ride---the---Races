...
function RideScreen({
  ...
  const detailNextName=upcomingBoundary.finish?(actions.next?.name??nextSectionTarget?.name??(engine.segmentIndex===segments.length-1?'FINISH':'ROAD AHEAD')):upcomingBoundary.name
  const detailChangeKm=coursePosition.distanceToNextGradientBoundary??upcomingBoundary.distance
        ...
        <>
          <div className="live-profile-card" aria-label={currentSegmentIsClimb ? "Live climb gradient profile" : "Live stage profile"}>
            ...
            {profileView.mode==='DETAIL' ? (
              <ProfileDetail4022 state={profileView} event={raceSituation} progress={localGradientProgress} gradientBlocks={mergedGradientBlocks} gradientIndex={detailGradientIndex} currentGradient={gateActive?0:activeGradient} nextGradient={gateActive?null:coursePosition.nextGradientSection?.gradient??mergedGradientBlocks[detailGradientIndex+1]?.gradient??nextGradient} nextName={gateActive?(firstRaceTarget?.name??'RACING SECTION 1'):detailNextName} changeDistance={massStart?.phase==='KILOMETRE_ZERO'?formatTime(massStart.kilometreZeroRemaining):gateActive?formatTime(massStart?.warmupRemaining??0):coursePosition.gradientBoundaryCrossing?'CHANGE NOW':formatDistance(Math.max(.001,detailChangeKm),measurementSystem)} resistance={displayResistance} context={courseContext}/>
            ) : showClimbView ? (
