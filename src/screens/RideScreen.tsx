...
function OutdoorRideCockpit({onBack,onCompletion}:{onBack:()=>void;onCompletion:(endedEarly:boolean)=>void}){
 const {career}=useCareer(),{ride,elapsed,pause,resume,setLocationEnabled,setJeanMuted,markJeanCues}=useOutdoorRide(),snapshot=outdoorWorkoutSnapshot(elapsed),requested=useRef(false),visualCue=currentOutdoorJeanCue(elapsed)
 ...
 useEffect(()=>{if(!ride)return;const pending=nextOutdoorJeanCue(elapsed,ride.deliveredJeanCueIds??[]);if(!pending.cue)return;markJeanCues(pending.consumedIds);speak(pending.cue.text)},[elapsed,ride?.id])
