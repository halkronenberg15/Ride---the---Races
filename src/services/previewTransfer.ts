...
export function buildCareerExport(account:RiderAccount,career:CareerState,activeRide:ActiveRide|null,sourceApplicationVersion:string,now=new Date().toISOString(),authenticatedPreview=false):CareerExport{
 if(account.role!=='owner'&&account.role!=='rider'&&!authenticatedPreview)throw new Error('Owner or authenticated preview access is required for Preview Data Transfer.')
...
export function readRecoveryBackup(account:RiderAccount,authenticatedPreview=false):RecoveryBackup|null{if(account.role!=='owner'&&account.role!=='rider'&&!authenticatedPreview)return null;try{const raw=localStorage.getItem(backupKey(account.id));return raw?JSON.parse(raw) as RecoveryBackup:null}catch{return null}}
...
export function commitCareerImport(account:RiderAccount,current:CareerState,currentActive:ActiveRide|null,preview:ImportPreview,confirmed:boolean,now=new Date().toISOString(),authenticatedPreview=false){
 if(account.role!=='owner'&&account.role!=='rider'&&!authenticatedPreview)throw new Error('Owner or authenticated preview access is required for Preview Data Transfer.')
 ...
 try{localStorage.setItem(backupKey(account.id),JSON.stringify(backup));localStorage.setItem(careerKey,JSON.stringify(preview.migrated));localStorage.setItem(rideKey,JSON.stringify(preview.payload.activeRide));if(account.role==='owner')replaceProgramEntitlements(account,preview.payload.programEntitlements??[]);return {career:preview.migrated,activeRide:preview.payload.activeRide,backup}}
 catch(error){if(priorCareer===null)localStorage.removeItem(careerKey);else localStorage.setItem(careerKey,priorCareer);if(priorRide===null)localStorage.removeItem(rideKey);else localStorage.setItem(rideKey,priorRide);if(priorBackup===null)localStorage.removeItem(backupKey(account.id));else localStorage.setItem(backupKey(account.id),priorBackup);if(account.role==='owner')replaceProgramEntitlements(account,account.entitlements);throw new Error(error instanceof Error?`Import failed: ${error.message}`:'Import failed; the existing career was preserved.',{cause:error})}
}
export function restoreRecoveryBackup(account:RiderAccount,confirmed:boolean,authenticatedPreview=false){if(account.role!=='owner'&&account.role!=='rider'&&!authenticatedPreview)throw new Error('Owner or authenticated preview access is required for Preview Data Transfer.');if(!confirmed)throw new Error('Explicit confirmation is required.');const backup=readRecoveryBackup(account,authenticatedPreview);if(!backup)throw new Error('No pre-import backup is available.');localStorage.setItem(careerStorageKey(account.id),JSON.stringify(migrateCareer(backup.career)));localStorage.setItem(activeRideStorageKey(account.id),JSON.stringify(backup.activeRide));if(account.role==='owner')replaceProgramEntitlements(account,backup.programEntitlements??account.entitlements);return backup}
