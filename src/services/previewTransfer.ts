...
export function buildCareerExport(account:RiderAccount,career:CareerState,activeRide:ActiveRide|null,sourceApplicationVersion:string,now=new Date().toISOString(),authenticatedPreview=false):CareerExport{
 if(account.role!=='owner'&&!authenticatedPreview)throw new Error('Owner or authenticated preview access is required for Preview Data Transfer.')
...
export function readRecoveryBackup(account:RiderAccount,authenticatedPreview=false):RecoveryBackup|null{if(account.role!=='owner'&&!authenticatedPreview)return null;try{const raw=localStorage.getItem(backupKey(account.id));return raw?JSON.parse(raw) as RecoveryBackup:null}catch{return null}}
