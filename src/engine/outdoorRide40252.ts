...
export const outdoorActivityId=(ride:ActiveOutdoorRide)=>`outdoor:${ride.id}`
export type ActiveOutdoorRide=PersistedRideClock&{id:string;assignmentId:string;startedAt:string;completionRequested?:'FULL'|'EARLY';locationEnabled?:boolean;deliveredJeanCueIds?:string[];jeanMuted?:boolean}
...
export const OUTDOOR_ENDURANCE_90_SECTIONS:OutdoorWorkoutSection[]=[
 ...
 {id:'aerobic-endurance',title:'Aerobic Endurance',durationSeconds:4200,rpe:'RPE 3–4/10',effort:'Conversational',guidance:'Steady aerobic riding. Keep your effort conversational; avoid threshold efforts and hard climbing.'},
 {id:'cooldown',title:'Cooldown',durationSeconds:600,rpe:'RPE 2–3/10',effort:'Easy',guidance:'Reduce effort gradually and finish with relaxed spinning.'},
]
export const OUTDOOR_ENDURANCE_90_SECONDS=OUTDOOR_ENDURANCE_90_SECTIONS.reduce((sum,section)=>sum+section.durationSeconds,0)
export function outdoorWorkoutSections(durationMinutes=90):OutdoorWorkoutSection[]{if(durationMinutes<20)throw new Error('Outdoor workout must allow warm-up and cooldown.');return OUTDOOR_ENDURANCE_90_SECTIONS.map(section=>section.id==='aerobic-endurance'?{...section,durationSeconds:(durationMinutes-20)*60}:section)}
export function outdoorWorkoutSnapshot(elapsedSeconds:number,durationMinutes=90){const sections=outdoorWorkoutSections(durationMinutes),total=durationMinutes*60;let start=0;for(let index=0;index<sections.length;index++){const section=sections[index],end=start+section.durationSeconds;if(elapsedSeconds<end||index===sections.length-1)return {current:section,upNext:sections[index+1]??null,currentRemaining:Math.max(0,end-elapsedSeconds),totalRemaining:Math.max(0,total-elapsedSeconds),complete:elapsedSeconds>=total};start=end}throw new Error('Outdoor workout has no sections.')}
...
/** Records each session once, preserving the original assignment and earlier attempts. */
export function completeOutdoorRide(state:Alpha4025State,ride:ActiveOutdoorRide,measurements:OutdoorMeasurements,completedAt:string,endedEarly=false):Alpha4025State{
 if(state.outdoorActivities.some(activity=>activity.id===outdoorActivityId(ride)))return state
 const plan=state.trainingPlan,assignment=plan?.weeks.flatMap(week=>week.assignments).find(item=>item.id===ride.assignmentId)
 if(!plan||!assignment||assignment.type!=='CYCLING'||assignment.environment!=='OUTDOOR'||!/^(outdoor-easy-60|outdoor-endurance-(75|90|120|150|180|210|240|270))$/.test(assignment.workoutId??''))throw new Error('Outdoor completion requires a scheduled Outdoor Endurance assignment.')
