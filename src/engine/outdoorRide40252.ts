...
export type ActiveOutdoorRide=PersistedRideClock&{assignmentId:string;startedAt:string}
export type OutdoorMeasurements={durationSeconds?:number;distanceMiles?:number;rpe?:number;averageHeartRate?:number;maximumHeartRate?:number;averageCadenceRpm?:number;averagePowerWatts?:number}

export const outdoorRideStorageKey=(accountId:string)=>`ride-the-races-active-outdoor-v1:${accountId}`
export function startOutdoorRide(assignmentId:string,now:number):ActiveOutdoorRide{return {assignmentId,startedAt:new Date(now).toISOString(),accumulatedSeconds:0,runningSince:now,paused:false}}
