...
test('new users are not Hal-seeded and legacy careers have an explicit migration path',()=>{const source=readFileSync(new URL('../state/careerPersistence.ts',import.meta.url),'utf8');assert.match(source,/name:''/);assert.match(source,/ftpKnown:false/);assert.match(source,/equipmentForDevices/);assert.match(source,/peloton-baseline-bike/);assert.match(source,/schemaVersion:4/)})
test('42 stages retain exact finish and mobile cockpit exposes safe areas and compact next target',()=>{for(const race of [tour2026,vuelta2026])for(const raw of race.stages){const stage=toRaceStage(race,raw),timed=applyDurationSelection(stage.segments,durationSelectionForStage(stage,{mode:'RECOMMENDED'}));const road=createRoadModel(stage.number,timed.segments,stage.distanceKm,undefined,stage.profilePoints,stage.officialCourseMarkers,stage.raceId,208);assert.equal(road.roadSnapshot(road.duration).courseProgress,1)}const ui=readFileSync(new URL('../screens/RideScreen.tsx',import.meta.url),'utf8');assert.match(ui,/var\(--safe-top\)/);assert.match(ui,/NEXT:/);assert.doesNotMatch(ui,/SELECTED COURSE DURATION/)})
