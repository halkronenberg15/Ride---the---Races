...
test('no-FTP calibration briefing renders EFFORT without provisional watts',()=>{const html=renderToStaticMarkup(createElement(PreviewTargetValues,{target:noFtpPresentation(calibration.stage.segments[0],PELOTON_BASELINE_EQUIPMENT)}));assert.match(html,/>EFFORT</);assert.match(html,/VERY EASY/);assert.match(html,/65–80 rpm/);assert.match(html,/25–30%/);assert.doesNotMatch(html,/POWER|PROVISIONAL|0–0 W|0–3%/)})
