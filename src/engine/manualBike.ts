...
export type ManualBikeTarget={targetPowerRange:{min:number;max:number};resolvedCadenceRange:{min:number;max:number};resolvedExactResistance:number|null;resolvedResistanceRange:{min:number;max:number}|null;preferredRoadResistance:number|null;predictedPowerRange:{min:number;max:number}|null;feasibility:TargetFeasibility;calibrationConfidence:'UNAVAILABLE'|CalibrationConfidence;cadenceAdjusted:boolean;resistanceAdjusted:boolean;adjustmentReason:string}
...
export function resolveManualBikeTarget(input:{powerRange:{min:number;max:number};cadenceRange:{min:number;max:number};gradient:number;equipment:EquipmentInstance;profile:ManualBikeProfile|null;preferences?:CadencePreferences}):ManualBikeTarget{
 ...
 if(overlaps()){const window=feasibleWindow(resistance)!;resistance=Math.round(clamp(resistance,window.min,window.max));return {targetPowerRange:input.powerRange,resolvedCadenceRange:cadence,resolvedExactResistance:resistance,resolvedResistanceRange:window,preferredRoadResistance:preferred,predictedPowerRange:power,feasibility:cadenceAdjusted?'ADJUSTED':'EXACT',calibrationConfidence:input.profile.calibrationConfidence,cadenceAdjusted,resistanceAdjusted:false,adjustmentReason:cadenceAdjusted?'Cadence preference applied within safe limits.':'Preferred road setting overlaps the physiological target.'}}
 ...
 return {targetPowerRange:input.powerRange,resolvedCadenceRange:cadence,resolvedExactResistance:resistance,resolvedResistanceRange:window??{min:resistance,max:resistance},preferredRoadResistance:preferred,predictedPowerRange:power,feasibility:feasible?'ADJUSTED':'LIMITED',calibrationConfidence:input.profile.calibrationConfidence,cadenceAdjusted,resistanceAdjusted:resistance!==preferred,adjustmentReason:feasible?'Physiology requires a safe adjustment from the road-feel anchor.':'Safe calibrated limits cannot fully overlap the power objective.'}
}
