...
test('Detail component renders canonical current, next, boundary, rider and resistance data',()=>{const html=renderToStaticMarkup(createElement(ProfileDetail4022,{state:detail,event:montrealRoadStory[2],progress:.35,gradientBlocks:[{start:0,end:.5,gradient:3.4},{start:.5,end:1,gradient:1.9}],gradientIndex:0,currentGradient:3.4,nextGradient:1.9,distanceToTransition:.42,resistance:'44–46%'}));assert.match(html,/CURRENT/);assert.match(html,/NEXT/);assert.match(html,/0–50%/);assert.match(html,/RIDER COORDINATE 35.0%/);assert.match(html,/0.42 km/);assert.match(html,/44–46%/)})
test('Overview suppresses detail while the accessible view control remains operable',()=>{assert.equal(renderToStaticMarkup(createElement(ProfileDetail4022,{state:overview,progress:0,gradientBlocks:[],gradientIndex:0,currentGradient:0,nextGradient:null,distanceToTransition:0,resistance:'0%'})),'');assert.match(renderToStaticMarkup(createElement(ProfileViewControl,{state:detail,onToggle:()=>{}})),/Switch profile to Overview view/)})
test('authored race markers move independently around one canonical rider coordinate',()=>{const a=situationGroups(montrealRoadStory[2],.3),b=situationGroups(montrealRoadStory[2],.4);assert(a.every((_,index)=>b[index].position>a[index].position));assert.notEqual(a.find(g=>g.id==='peloton')?.position,.3)})
