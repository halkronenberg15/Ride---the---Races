...
import { ManualResistanceAnnouncementGate, PELOTON_BASELINE_EQUIPMENT, PELOTON_MANUAL_PROFILE, calibrationSampleFromStableWindow, estimateBikePower, inverseResistanceForPower, resolveVirtualRoadLoad, translateManualRoadFeel } from './manualBike.ts'
...
test('manual road feel matrix is deterministic, bounded and terrain-led',()=>{
 ...
 assert.ok(states.every(state=>state.manualTarget.feasibility==='EXACT'||state.manualTarget.feasibility==='ADJUSTED'||state.manualTarget.feasibility==='LIMITED'))
 assert.ok(states[1].cadenceRange.min>=90)
})

test('device profile preserves aggregate provenance and stable telemetry boundary',()=>{
 assert.equal(PELOTON_MANUAL_PROFILE.calibrationConfidence,'BASELINE');assert.ok(PELOTON_MANUAL_PROFILE.calibrationSamples.every(sample=>sample.sourceType==='manual-calibration'&&sample.aggregate&&sample.confidence==='MEDIUM'))
...
test('FTP, strategy and duration never alter fixed-position geography',()=>{const at=40;const snapshots:RoadSnapshot[]=[];for(const ftp of [180,208,250])for(const strategy of ['Conservative','Balanced','Aggressive'] as const)for(const mode of ['STANDARD','RECOMMENDED'] as const){const selection=durationSelectionForStage(stage,{mode});const segments=applyDurationSelection(adaptSegments(stage.segments,ftp,strategy),selection).segments;const road=createRoadModel(stage.number,segments,stage.distanceKm,undefined,stage.profilePoints,stage.officialCourseMarkers,stage.raceId,ftp);snapshots.push(road.roadSnapshot(road.elapsedAtCourseDistance(at)))}assert.ok(snapshots.every(snapshot=>snapshot.gradient===snapshots[0].gradient&&snapshot.elevation===snapshots[0].elevation))})

test('workout CSV evidence remains equipment-scoped and aggregate-low-confidence',async()=>{const {calibrationSamplesFromWorkoutCsv,withRiderCalibration}=await import('./pelotonCalibration.ts');const csv='Workout Timestamp,Avg Cadence,Avg Resistance,Avg Watts\n2026-08-01,88,36,105\n2026-09-01,85,37,104';const samples=calibrationSamplesFromWorkoutCsv(csv,'peloton-baseline-bike');assert.equal(samples.length,2);assert.ok(samples.every(sample=>sample.aggregate&&sample.confidence==='LOW'&&sample.equipmentId==='peloton-baseline-bike'));assert.equal(samples[0].observedAt,'2026-09-01');const personalized=withRiderCalibration(PELOTON_BASELINE_EQUIPMENT,samples);assert.equal(personalized.calibrationConfidence,'PERSONALIZED');assert.equal(personalized.calibrationSamples?.length,2)})

test('normal Peloton onboarding and legacy migration attach and use anonymous seed calibration',async()=>{const {equipmentForDevices,migrateCareer}=await import('../state/careerPersistence.ts');const {bikeProfileForEquipment}=await import('./manualBike.ts');const equipment=equipmentForDevices(['Peloton']);const bike=equipment.instances[0];assert.equal(bike.calibrationConfidence,'PERSONALIZED');assert.deepEqual(bike.calibrationSamples?.map(sample=>[sample.cadence,sample.resistance,sample.power]),[[88,36,105]]);assert.equal(bikeProfileForEquipment(bike)?.calibrationConfidence,'PERSONALIZED');const legacy=migrateCareer({rider:{devices:['Peloton']} as never,equipment:{activeEquipmentId:'old-bike',connectionMethod:'manual-guidance',instances:[{...bike,id:'old-bike',calibrationConfidence:'BASELINE',calibrationSamples:undefined}]} as never});assert.equal(legacy.equipment.instances[0].calibrationSamples?.[0].equipmentId,'old-bike')})
