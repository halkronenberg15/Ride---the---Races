...
test('A: release package metadata is exactly Alpha 4.0.22', () => {
  const packageJson = JSON.parse(readFileSync(new URL('../../package.json', import.meta.url), 'utf8'))
  const lock = JSON.parse(readFileSync(new URL('../../package-lock.json', import.meta.url), 'utf8'))
  assert.equal(packageJson.version, '4.0.22'); assert.equal(lock.version, '4.0.22'); assert.equal(lock.packages[''].version, '4.0.22')
})
