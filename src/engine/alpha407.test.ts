...
test('La Vuelta stages 1–9 are complete synchronized-engine stage definitions',()=>{
  assert.equal(vuelta2026.stages.length,21); assert.equal(vueltaRideStages.length,9)
  for(const stage of vueltaRideStages){assert.ok(stage.segments.length>=5);assert.ok(stage.segments.every(s=>s.sec>0&&s.power&&s.cadence&&s.resistance));assert.ok(vuelta2026.stages[stage.number-1].rideable)}
...
test('course markers use metadata colors and fixed upward-only local geometry',()=>{
  ...
  assert.equal(model.markers.find(m=>m.type==='kom')?.color,raceIdentities['vuelta-2026'].komColor)
  assert.equal(model.markers.find(m=>m.type==='sprint')?.color,raceIdentities['vuelta-2026'].pointsColor)
