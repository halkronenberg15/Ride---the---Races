...
test('rendered calibration helper contains one dynamic starting pairing',()=>{const html=renderToStaticMarkup(createElement(CalibrationHelper,{resistance:38,cadence:91,guidance:'LOW: +1 · HIGH: −1'}));assert.equal(html.match(/START 38% @ 91 RPM/g)?.length,1);assert.match(html,/LOW: \+1 · HIGH: −1/)})
