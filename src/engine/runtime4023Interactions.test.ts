...
import { WorldsOverviewLabels, ProfileDetail4022, LiveTrackerHeader4023, WorldsGroupMarkers, TacticalStatusStrip } from '../components/WorldsRaceLayer.ts'
...
test('Detail reports an honest bounded state when no canonical gradient boundary remains',()=>{const html=renderToStaticMarkup(createElement(ProfileDetail4022,{state:{mode:'DETAIL',activeRangeId:'road',autoConsumedIds:[]},progress:.01,gradientBlocks:[],gradientIndex:0,currentGradient:0,nextGradient:null,nextName:'NO UPCOMING GRADIENT CHANGE',changeDistance:null,resistance:'38–41%',context:'Brossard Rollout'}));assert.match(html,/NO UPCOMING GRADIENT CHANGE/);assert.doesNotMatch(html,/Avenue du Parc Finish|168\.5 mi|CHANGE IN/)})

test('Finish label is high contrast and contained while endpoint remains canonical',()=>{const html=renderToStaticMarkup(createElement('svg',null,createElement(CourseFinishMarker4023,{y:42})));assert.match(html,/x="98"/);assert.match(html,/fill="#ffffff"/);assert.match(html,/stroke="#050505"/);assert.match(html,/data-endpoint-x="100"/);assert.match(html,/x1="100" y1="42"/)})
