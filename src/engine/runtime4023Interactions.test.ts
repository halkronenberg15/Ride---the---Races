...
test('Finish label is solid, backed and contained without Safari-distorting text strokes',()=>{const html=renderToStaticMarkup(createElement('svg',null,createElement(CourseFinishMarker4023,{y:42}))),textTag=html.match(/<text[^>]*>/)?.[0]??'';assert.match(html,/<rect[^>]*fill="rgba\(5,5,5,\.78\)"/);assert.match(textTag,/x="97"/);assert.match(textTag,/fill="#ffffff"/);assert.doesNotMatch(textTag,/stroke|paint-order|paintOrder/);assert.match(html,/data-endpoint-x="100"/);assert.match(html,/x1="100" y1="42"/)})
