...
test('authenticated Team Bus preview renders required plan labels without a lock',()=>{const html=renderToStaticMarkup(createElement(OffSeasonPreviewButton,{authenticated:true,plan:generatePlan(HAL_INPUT,HAL_GOALS),onOpen:()=>{}}));for(const label of ['OFF-SEASON TRAINING','PREVIEW ACCESS','Start date: September 21, 2026','Plan length: 12 weeks','Open Off-Season Plan'])assert.match(html,new RegExp(label));assert.doesNotMatch(html,/🔒|disabled/)})
