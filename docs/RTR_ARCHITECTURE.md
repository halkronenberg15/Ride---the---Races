...
## Alpha 4.0.20 lifecycle, timeline, and equipment contract
...
Alpha 4.0.25 adds account-scoped schema 6 and an authoritative rider-development boundary: intake → explained profile → series entitlement → immutable season archive/review → questionnaire → selected camps → one 12-week plan → calendar completion/adaptation. Completed history is immutable during regeneration. Series filtering never grants administrative authority. Femmes development fixtures require explicit verification metadata before production use. Outdoor targets without valid power use heart rate, talk test and plain effort; power-meter installation starts a calibration period rather than asserting indoor/outdoor equivalence. See `docs/ALPHA_4.0.25.md` for the complete contract and deferred integrations.

Origin-scoped preview storage is bridged only by an owner-initiated local JSON export/replacement import. The envelope excludes authentication and session material; destination account identity remains authoritative while career identity and safe entitlements are rebound. Validation precedes any write, a per-account recovery snapshot precedes replacement, and standard schema migration normalizes the result. Readiness has one categorical, local-date-aware projection and deliberately has no composite physiological score.
...
The selected duration means the complete planned workout. Normally its final five minutes are reserved for recovery; rollout, protected KM0, and racing occupy the remainder. Canonical geography reaches 100% at `raceFinishTime`, where official time, distance, results, points, and tactical outcome freeze before lifecycle enters `OPTIONAL_COOLDOWN`. Skip and End controls may shorten cooldown without stretching racing. Cooldown metrics are stored separately and progressive recovery targets still pass through the equipment resolver.
...
Career persistence is schema version 4; the migration preserves every Alpha 4.0.19 field and adds only tactical/radio/marker, early-termination, and calendar restoration state. Equipment instances own capabilities, calibration reference/confidence, and active selection. The Peloton-family reference is a low-confidence `BASELINE`, separate from Hal's historical aggregate evidence; it is not personalized or calibrated. Unsupported manual bikes retain FTP-based power and cadence but receive no Peloton-native resistance. Smart equipment records capability without claiming a control adapter. New careers have no borrowed identity and an unknown, unverified FTP; migration retains existing careers, including Hal's stored career.
...
### Alpha 4.0.20 field-test invariants

Selected duration is one monotonic clock. After neutral rollout and protected Kilometre Zero, each official-racing timestamp resolves exactly one canonical course coordinate. Sector bounds, profile position, gradient, climb geometry, Sprint/KOM markers, finish, and tactical availability are projections of that coordinate; none may independently scale time or distance. Every future-event ETA is bounded by selected ride time remaining.

Peloton guidance is a feasible resistance window (normally three points wide) plus a starting value. Every value in the window overlaps some safe cadence/power combination. The resolver priority is safety, physiology, road feel, cadence preference, then minimal knob movement. `LIMITED` is reserved for a genuinely absent complete intersection and its reason belongs in Ride Details.

Tactics are deterministic persistent modifiers over prescription, never geography. Returning to Peloton is a 45-second progressive transition. Team Radio uses a priority queue and persistent history; temporary closed captions occupy reserved profile space. Early termination records the canonical snapshot and proportional credit without completing or advancing the stage. Calendar restoration retains month context and navigation position. Portrait and landscape are separate compositions and orientation is never ride state.
